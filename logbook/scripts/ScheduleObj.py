...
class Countdown:
    ...
    def __init__(self, start_time, end_time):
        self.duration_expired = False  # False = countdown not expired
        self.countdown_expired = False  # False = countdown not expired
        self.start_time = start_time
        self.end_time = end_time
        self.countdown = self.room_countdown()  # initial value
        self.duration = self.calculate_duration()  # initial value
    ...
    @staticmethod
    def check(countdown):
        if countdown is None:  # countdown expired
            return True

        if countdown <= datetime.timedelta(seconds=1):  # countdown expired
            return True
